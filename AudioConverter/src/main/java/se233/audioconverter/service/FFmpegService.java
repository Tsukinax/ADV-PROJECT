    ...
    public void convertAudio(AudioFile audioFile, ConversionSettings settings,
        ...
        try {
            ...
            FFmpegBuilder builder = new FFmpegBuilder()
                    .setInput(audioFile.getFilePath())
                    .overrideOutputFiles(true)
                    .addOutput(outputFilename)
                    .setFormat(settings.getOutputFormat().getExtension())
                    .setAudioCodec(settings.getOutputFormat().getCodec())
                    .setAudioChannels(settings.getChannels().getCount())
                    .setAudioSampleRate(settings.getSampleRate().getRate())
                    .setAudioBitRate(settings.getQuality().getBitrate() * 1000L)
                    .done();
